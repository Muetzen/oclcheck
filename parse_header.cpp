...
ParseHeader::printErrorStringMethod (void)
{
    ...
    {
        std::cout << "\tcase " << mClErrorDefines [i] << ": return \"" << mClErrorDefines [i] << "\";\n";
    }
    std::cout << "\t}\n";
...
ParseHeader::skipMultiLineComment (void)
{
    ...
    while (mFile.eof () == false)
    {
        ...
        {
            if (isspace (mLastComment [0]))
            {
                mLastComment.erase (0, 1);
            }
            if (mLastComment.empty () == false && isspace (mLastComment [mLastComment.length () - 1]))
            {
                mLastComment.erase (mLastComment.length () - 1);
            }
            // read '/'
            mFile.get (c);
            return std::string ();
