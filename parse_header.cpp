...
std::string
ParseHeader::printErrorCodeTable (void)
{
    std::string ret;

    std::cout << "// This file is automatically generated, do not edit.\n\n";

    std::cout << "static struct clErrorCodes\n";
    std::cout << "{\n";
    std::cout << "\tcl_int mValue;\n";
    std::cout << "\tconst char * mName;\n";
    std::cout << "} gClErrorCodes [] = \n";
    std::cout << "{\n";

    for (size_t i = 0; i < mClErrorDefines.size (); ++i)
    {
        std::cout << "\t{ " << mClErrorDefines [i] << ", \"" << mClErrorDefines [i] << "\" },\n";
        // Note: Remove ',' after last element.
    }
    std::cout << "};\n";
...
ParseHeader::parseFile (void)
{
    ...
    while (mFile.eof () == false)
    {
        ...
        if (token == "#define")
        {
            ...
            // TODO: this assumes that the OpenCL error codes are either CL_SUCCESS or negative.
            // It will not work correctly, if there are any other #define's starting with '-' in cl.h.
            if (name == "CL_SUCCESS" || value [0] == '-')
            {
                mClErrorDefines.push_back (name);
            }
